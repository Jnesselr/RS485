...
unsigned int FilterByValue::lookAheadBytes() {
  return lookAhead;
}

bool FilterByValue::preFilter(const RS485BusBase& bus, const int startIndex) {
  return preValues.isSet(bus[startIndex + lookAhead]);
}

bool FilterByValue::postFilter(const RS485BusBase& bus, const int startIndex, const int endIndex) {
...
bool ValueFilter::isSet(uint8_t byte) {
  uint8_t lookAhead = byte >> 3;
  uint8_t shift = byte & 0x7;
